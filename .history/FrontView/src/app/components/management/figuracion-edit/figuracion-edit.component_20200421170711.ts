...
export class FiguracionEditComponent implements OnInit {
  ...
  deleteManoPage() {
    ...
    this.urlsManos.splice(this.indexDeleteTatuaje, 1);
    this.manosFileList.splice(this.indexDeleteTatuaje, 1);
    this.listManos.splice(this.indexDeleteTatuaje, 1);
  }
  ...
  actualizarCoche(idUser) {
    ...
    for (let i = 0; i < filesAmount; i++) {
      ...
      if (this.listCoches[i].idCoche === 0) {
        ...
        let url = 'figuracion/' + dnuIser + '/coche-' + this.cochesFileList[indexFileCoche].name;
        indexFileCoche++;
        coche.fotoCoche = url;
        /***GUARDE CON SERVICIO */
        ...
        this.authService.findByIdCoche(coche).subscribe(
          resp => {
  ...
  actualizarMoto(idUser) {
    ...
    for (let i = 0; i < filesAmount; i++) {
      ...
      if (this.listMotos[i].idMoto === 0) {
        let dnuIser = this.childForm.get('numeroDNI').value;
        let urlMoto = 'figuracion/' + dnuIser + '/moto-' + this.motosFileList[indexFileMoto].name;
        indexFileMoto++;
  ...
  actualizarTatuajes(idUser) {
    ...
    for (let i = 0; i < filesAmount; i++) {
      ...
      if (this.listTatuajes[i].idFotoTatuaje === 0) {
        let dnuIser = this.childForm.get('numeroDNI').value;
        let url = 'figuracion/' + dnuIser + '/tatuajes-' + this.tatuajeFileList[index].name;
        index++;
  ...
  actualizarManos(idUser) {
    ...
    for (let i = 0; i < filesAmount; i++) {
      ...
      if (this.listManos[i].idFotoMano === 0) {
        let dnuIser = this.childForm.get('numeroDNI').value;
        let url = 'figuracion/' + dnuIser + '/tatuajes-' + this.manosFileList[index].name;
        index++;
  ...
  subirArchivos() {
    ...
    if (this.cochesFileList != null) {
      ...
      for (let i = 0; i < filesAmount; i++) {
        let urlCoche = 'figuracion/' + idUser + '/coche-' + this.cochesFileList[i].name;
