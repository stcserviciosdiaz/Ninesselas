...
export class FiguracionEditComponent implements OnInit {
  ...
  pasarFotos() {
    ...
    /*for (var x = 0; x < this.listMotos.length; x++) {
      console.log('MOTOS: ' + this.listMotos[x].fotoMoto);

      ref = this.storage.ref(this.listMotos[x].fotoMoto);
      ref.getDownloadURL().subscribe(resp => {
        console.log('DATOS: ' + resp);
        this.listMotos[x].fotoMoto = resp;
  ...
  verImagenFB(filePath): Observable<string> {

    let imageResponse: Observable<string>;
    let ref = this.storage.ref(filePath);
    ref.getDownloadURL().subscribe(resp => {
      imageResponse = resp;
      //this.urlsMotos.push(resp);
