...
export class FiguracionEditComponent implements OnInit {
  ...
  pasarFotos() {
    ...
    console.log('MOTOS: ' + this.listMotos.length);

    //} else { filePath = 'admin/9999999999/image-not-found.png'; }

    for (var x = 0; x < this.listMotos.length; x++) {

      console.log('PIC BASE: ' + this.listMotos[x].fotoMoto);
      this.listMotos[x].fotoMoto = this.verImagenFB(this.listMotos[x].fotoMoto);
      console.log('PIC FB: ' + this.listMotos[x].fotoMoto);


    }

    console.log('OBS MOTOS: ' + this.listMotos.length);
  ...
  verImagenFB(filePath): any {

    let imageResponse = '';
    let ref = this.storage.ref(filePath);
    return ref.getDownloadURL().subscribe(resp => {
      console.log('RESO IMAGEN FB:' +resp);
      return resp;
    });
