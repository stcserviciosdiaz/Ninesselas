...
export class FiguracionEditComponent implements OnInit {
  ...
  verNombreCochePage(indexPage: any): string {
    ...
    return nombreImagen.toString();
  }

  indexDeleteCoche: any;
  confirmDeleteCoche(index: any) {
    indexDeleteCoche: index;
  }

  deleteCochePage() {

    if (this.listCoches[this.indexDeleteCoche].idCoche !== 0) {
      this.authService.deleteCoche(this.listCoches[this.indexDeleteCoche])
        .subscribe(resp => {
        });
    }
