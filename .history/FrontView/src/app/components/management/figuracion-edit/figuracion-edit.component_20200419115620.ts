...
export class FiguracionEditComponent implements OnInit {
  ...
  pasarFotos() {
    ...
    /**FOTO DE MOTO */
    var storageRef = this.storage.storage.ref("/figuracion/0401397948");
    ...
    function displayImage(imageRef) {
      imageRef.getDownloadURL().then(function(url) {
        ...
        this.obsMotos.push(url);
      }).catch(function(error) {
        // Handle any errors
      });
    }

    console.log('TAMA:O MOTOS: '+this.obsMotos.length);
    ...
    if (this.usuario.motoList !== null && this.usuario.motoList.pop().fotoMoto !== null) {
      ...
      console.log('MOTOS: ' + motosPath.length);

    } else { filePath = 'admin/9999999999/image-not-found.png'; }

    for (var x = 0; x < motosPath.length; x++) {
      console.log('MOTOS: ' + motosPath[x].fotoMoto);

      ref = this.storage.ref(motosPath[x].fotoMoto);
      ref.getDownloadURL().subscribe(resp => {
