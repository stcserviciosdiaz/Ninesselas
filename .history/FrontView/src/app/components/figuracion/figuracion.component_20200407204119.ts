...
export class cocheView { coche: coche; url: string; file: File; fileView: any }
...
export class FiguracionComponent implements OnInit {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      for (let i = 0; i < filesAmount; i++) {
        ...
        if (fileUpload !== null) {
          var reader = new FileReader();
          reader.onload = (event1: any) => {
            this.listCoche.push(null, null, event.target.files[i], event1.target.result);
  ...
  aceptarCoche(indexCocheFile: any) {
    ...
    console.log('DATOS COCHE ARRAY: ' + JSON.stringify(this.listCoche[indexCocheFile]));
