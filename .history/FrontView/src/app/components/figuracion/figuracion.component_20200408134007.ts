...
export class FiguracionComponent implements OnInit {
  ...
  listCoches: coche[] = [];
  listCocheAll:{url:any;file:File}[]=[];
  ...
  onFileCocheSelected(event) {
      ...
      for (let i = 0; i < filesAmount; i++) {
        ...
        if (fileUpload !== null) {
          ...
          reader.onload = (event1: any) => {
            ...
            this.listCocheAll.push({url:event1.target.result,file:event.target.files[i]});
          }
          reader.readAsDataURL(event.target.files[i]);
        }
      }
    ...
    console.log('listCocheall: '+JSON.stringify(this.listCocheAll));
  }
