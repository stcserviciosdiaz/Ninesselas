...
export class ChildrenComponent implements OnInit {
  ...
  guardarUltimosTrabajos(idUser) {
    ...
    for (let x = 0; x < ultimosTrabajosList.length; x++) {
      this.authService.saveUltimosTrabajos(ultimosTrabajosList[x]).subscribe(
        resTalla => {
          console.log('ut guardada');
        },
        (err) => {
          console.log(JSON.stringify(err));
        });
    ...
    if (this.familyBookFiles.length > 0) {
      ...
      for (let i = 0; i < filesAmount; i++) {
        let libroFamilia = {
          ...
          nombreFotoLibro: this.libroFamiliarList[i].nombreFotoLibro,
          urlFotoLibro: this.libroFamiliarList[i].urlFotoLibro,
          idUser: idUser
        };

        this.authService.saveLibroFamiliar(libroFamilia).subscribe(
          resTalla => {
            console.log('libro familiar guardada');
          },
          (err) => {
            this.ngxSmartModalService.create('confirm', 'Se ha presentado un Error, vuelva a intentarlo y si el problema persiste, contáctenos').open();
          });
...
console.log(JSON.stringify(this.childForm.value));
    ...
    this.authService.signup2(this.usuario).subscribe(
      res => {
        if (res.nombreArtistico !== 'ENCONTRADO') {
          ...
          this.guardarLibroFamiliar(res.idUser);
          this.guardarUltimosTrabajos(res.idUser);
